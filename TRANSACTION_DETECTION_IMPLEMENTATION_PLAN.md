...
| T3.2 | ✅ | Header, whole-sender, email-domain (with subdomains) and package lookup. A known sender is the verified route. **Content signal (core v0.7.0):** an unknown business header (never a phone number) whose body names exactly one institution, by a unique multi-word name from the pack or an IFSC code, is resolved unverified: capped at medium, so it goes to review. Anything else unknown is still ignored (`unknown_sender`), as the gap-doc rows require. The native pre-filter now keeps such messages too (body names and IFSC prefixes sent from the pack; JVM tests). Corpus cases for both routes |
...
| T3.12 | ✅ | Core flags `possible_manual_duplicate` from the recent-transaction digest and caps the item at medium. The server checks too: a device item with the same amount, currency and direction as a manual transaction within ±1 day goes to review with `possible_manual_duplicate`. The lookup is folded into the user query, so `/sync` stays at 10 queries. Transfers and committed imports are not checked |
...
| T4.2 | ✅ (SMS markets) | Idempotent importers recorded in `kb_import_runs` with source, licence and fetch time: `seed-pack`, `iso4217`, `csv-institutions`, `csv-merchants`, `csv-mcc`. Coverage report per country (`npm run kb -- coverage`). **Registry importers** download and convert a source's own format: `ifsc` (razorpay/ifsc bank names, the IFSC prefix of every Indian bank, which feeds the T3.2 content signal), `fdic` (FDIC BankFind) and `nsi-wikidata` (Name Suggestion Index brands with Wikidata ids). They add new rows in `review` for an admin to publish and never overwrite a curated row; an existing institution matched by code or name only gains a missing BIC or code. A weekly job refreshes the registries of countries the catalog serves. The formats were tested against fixtures written from each source's documentation; the hosts are not reachable from this environment. **Loaded through CSV export instead:** RBI and NPCI (HTML pages), India DLT headers (no public list), GLEIF (bulk archive), NCUA (zip), FCA (keyed API), ECB and BCB |
...
|---|---|---|---|
| `/sync`, 100 items (service + DB), p50 / p95 over 30 batches | 52–55 ms / 112–117 ms warm; 509 ms p95 on a cold database right after a restart | p95 ≤ 300 ms | ✅ warm |
...
| Admin dashboard | 4 queries, 6–7 ms warm: status, source and country totals in one `GROUPING SETS` scan, the institution table, the daily series, and the adoption row | ≤ 5 indexed | ✅ |
