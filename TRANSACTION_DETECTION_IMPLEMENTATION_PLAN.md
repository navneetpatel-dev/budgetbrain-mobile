...
| T6.3 | 🟡 | `/integrations` removed from backend, web and mobile; its paths answer 410 Gone. **Open:** the migration that moves pending `parsed_transactions` rows and erases `raw_content` is written but held for approval, because the erasure can't be undone. The table is dropped in a later release |
